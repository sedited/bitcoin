    ...
    BlockValidationState& state)
{
    ...
    m_chainman.time_check += time - time_start;
    LogDebug(BCLog::BENCH, "      - Spend Block processed %u transactions: %.2fms (%.3fms/tx) [%.2fs (%.2fms/blk)]\n", (unsigned)block.vtx.size(),
             Ticks<MillisecondsDouble>(time - time_start),
-            block.vtx.size() == 0 ? 0 : Ticks<MillisecondsDouble>(time - time_start) / block.vtx.size(),
             Ticks<SecondsDouble>(m_chainman.time_connect),
    ...
    DisconnectedBlockTransactions& disconnectpool)
{
             ...
             Ticks<MillisecondsDouble>(time_2 - time_1));
    {
        ...
        if (!res) {
            ...
            LogError("%s: BIP30Validate %s failed, %s\n", __func__, pindexNew->GetBlockHash().ToString(), state.ToString());
            return false;
        }
        CBlockUndo blockundo;

...
static const std::chrono::duration<double> RECONNECT_TIMEOUT_START{1.0};
/** Exponential backoff configuration - growth factor */
static const double RECONNECT_TIMEOUT_EXP = 1.5;
/** Maximum reconnect timeout in seconds to prevent excessive delays */
static const std::chrono::duration<double> RECONNECT_TIMEOUT_MAX{600.0};
...
bool TorControlConnection::Connect(const std::string& tor_control_center)
{
    ...
    std::optional<CService> control_service = Lookup(tor_control_center, 9951, fNameLookup);
    if (!control_service) {
    ...
    while (!m_interrupt) {
        ...
        if (!m_conn.IsConnected()) {
            ...
            if (!m_conn.Connect(m_tor_control_center)) {
                LogWarning("tor: Initializing connection to Tor control port %s failed", m_tor_control_center);
                if (!m_reconnect) {
                    break;
                }
...
/**
 * TODO: TCD if introducing a global is the preferred approach here since we
 * usually try to avoid them. We could let init manage the lifecycle or make
 * this part of NodeContext maybe instead.
 */

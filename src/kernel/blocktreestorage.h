...
public:
    BlockTreeStore(const fs::path& path, bool wipe_data = false);
    ...
    void WriteBatchSync(const std::vector<std::pair<int, const CBlockFileInfo*>>& file_info, const std::vector<CBlockIndex*>& block_info)

...
bool BlockManager::LoadBlockIndexDB(const std::optional<uint256>& snapshot_blockhash)
{
    ...
    for (std::set<int>::iterator it = setBlkDataFiles.begin(); it != setBlkDataFiles.end(); it++) {
        ...
        if (!m_block_store->MakeBlockStorageReader(pos)->IsValid()) {
            return false;
        }
    }
...
bool BlockManager::ReadBlockUndo(CBlockUndo& blockundo, const CBlockIndex& index) const
{
    ...
    auto reader{PolymorphicReaderWrapper{m_undo_store->MakeBlockStorageReader(pos)}};
    if (!reader.IsValid()) {
...
bool BlockManager::WriteBlockUndo(const CBlockUndo& blockundo, BlockValidationState& state, CBlockIndex& block)
{
    ...
    if (block.GetUndoPos().IsNull()) {
        ...
        auto writer{PolymorphicWriterWrapper{m_undo_store->MakeBlockStorageWriter(pos)}};
        if (!writer.IsValid()) {
...
FlatFilePos BlockManager::WriteBlock(const CBlock& block, int nHeight)
{
    ...
    auto writer{PolymorphicWriterWrapper{m_block_store->MakeBlockStorageWriter(pos)}};
    if (!writer.IsValid()) {
...
BlockManager::BlockManager(const util::SignalInterrupt& interrupt, Options opts)
    : m_prune_mode{opts.prune_target > 0},
      ...
      m_undo_store{std::make_unique<VecBlockStore>()},
      m_block_store{std::make_unique<VecBlockStore>()},
      // m_undo_store{std::make_unique<FileBlockStore>(m_opts.blocks_dir, "rev", UNDOFILE_CHUNK_SIZE, m_obfuscation)},
      // m_block_store{std::make_unique<FileBlockStore>(m_opts.blocks_dir, "blk", m_opts.fast_prune ? 0x4000 /* 16 kB*/ : BLOCKFILE_CHUNK_SIZE, m_obfuscation)},

...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    auto coins_tx_to{coins.AccessCoins(CTransaction(txTo))};
    BOOST_CHECK_EQUAL(GetP2SHSigOpCount(CTransaction(txTo), std::span{coins_tx_to}), 22U);
    ...
    auto coins_tx_non_std1{coins.AccessCoins(CTransaction(txToNonStd1))};
    BOOST_CHECK_EQUAL(GetP2SHSigOpCount(CTransaction(txToNonStd1), std::span{coins_tx_non_std1}), 16U);
    ...
    auto coins_tx_to_non_std2{coins.AccessCoins(CTransaction(txToNonStd2))};
    BOOST_CHECK_EQUAL(GetP2SHSigOpCount(CTransaction(txToNonStd2), std::span{coins_tx_to_non_std2}), 20U);
}

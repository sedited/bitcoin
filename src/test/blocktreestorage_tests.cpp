...
BOOST_AUTO_TEST_CASE(BlockTreeStoreRW)
{
    ...
    auto blocks{BlockMapToVector(test_map)};
    store.WriteBatchSync(fileinfo, {});
    node::BlockMap verify_map;
    BOOST_CHECK(store.LoadBlockIndexGuts(
        params->GetConsensus(),
        [&](const uint256& hash) { return InsertBlockIndex(verify_map, hash); },
        m_interrupt));
    CheckBlockMap(verify_map, blocks);
    CheckBlockFileInfo(0, info, store);
    store.ReadLastBlockFile(last_block);
    BOOST_CHECK_EQUAL(last_block, 1);

    // Write an empty file info vector
    WriteAndCheckBlockIndex(store, test_map, {}, m_interrupt, *params);
    CheckBlockFileInfo(0, info, store);
    store.ReadLastBlockFile(last_block);
    BOOST_CHECK_EQUAL(last_block, 1);
